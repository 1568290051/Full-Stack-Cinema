      ...
      <div class="sy-nav-down clearfix">
        <div class="sy clearfix" style="display: none;">
          ...
          <dl class="clearfix">
            ...
            <dd @click="query($event)">
              <a href="inlandVariety" class="on">内地综艺</a>
            </dd>
            <dd @click="query($event)">
  ...
  methods: {
    ...
    async getMovieList() {
      const { data: res } = await this.$http.get(
        "/selectVideo?video_sort_id=3&type_sort_id=13&time_sort_id=&region_sort_id="
      );
      console.log(res);
      // 如果没有成功，提示一下用户
      if (res.code !== 200) {
    ...
    async whole(event) {
      ...
      if(this.area2_id == '') {
        ...
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=3&type_sort_id=13&time_sort_id=&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
    ...
    async years(event, id) {
      ...
      if(this.area2_id == '') {
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=3&type_sort_id=13&time_sort_id="+this.time2_id+"&region_sort_id="
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
        ...
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=3&type_sort_id=13&time_sort_id="+this.time2_id+"&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
