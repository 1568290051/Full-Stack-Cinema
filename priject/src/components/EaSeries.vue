      ...
      <div class="sy-nav-down clearfix">
        <div class="sy clearfix" style="display: none;">
          ...
          <dl class="clearfix">
            ...
            <dd @click="query($event)">
              <a href="domesticSeries">国产剧</a>
            </dd>
            <dd @click="query($event)">
              <a href="htSeries">港台剧</a>
            </dd>
            <dd @click="query($event)">
              <a href="jkSeries">日韩剧</a>
            </dd>
            <dd @click="query($event)">
              <a href="eaSeries" class="on">欧美剧</a>
            </dd>
          </dl>
          <!-- 年代 -->
          <dl class="clearfix">
  ...
  methods: {
    ...
    async getMovieList() {
      const { data: res } = await this.$http.get(
        "/selectVideo?video_sort_id=2&type_sort_id=12&time_sort_id=&region_sort_id="
      );
      console.log(res);
      // 如果没有成功，提示一下用户
      if (res.code !== 200) {
    ...
    async whole(event) {
      ...
      if(this.area2_id == '') {
        ...
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=2&type_sort_id=12&time_sort_id=&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
    ...
    async years(event, id) {
      ...
      if(this.area2_id == '') {
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=2&type_sort_id=12&time_sort_id="+this.time2_id+"&region_sort_id="
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
        ...
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=2&type_sort_id=12&time_sort_id="+this.time2_id+"&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
    ...
    async region(event, region_id) {
      ...
      if(this.time2_id == '') {
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=2&type_sort_id=11&time_sort_id="+"&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
        ...
        const { data: res } = await this.$http.get(
          "/selectVideo?video_sort_id=2&type_sort_id=11&time_sort_id="+this.time2_id+"&region_sort_id="+this.area2_id
        );
        console.log(res);
        // 如果没有成功，提示一下用户
        if (res.code == 201) {
